...
for filepath in files:
    ...
    # We need to replace the entire exportToPDF function.
    # The function starts at `window.exportToPDF = function() {` and ends with the next function `window.generarCotizacion = function`
    
    start_str = "window.exportToPDF = function() {"
    end_str = "window.generarCotizacion = function"
    
    start_idx = content.find(start_str)
    end_idx = content.find(end_str)
    
    if start_idx != -1 and end_idx != -1:
        new_func = """window.exportToPDF = function() {
        ...
        html2pdf().set(opt).from(container.children[0]).save().then(() => {
            ...
            alert(`Cotización PDF para ${cot.codigo} generada con éxito.`);
        });
    };
    ...
    """
        ...
        # Also bump index.html and templates to v=26
        content = content.replace('main.js?v=25', 'main.js?v=26')
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)

# Update index html files for cache busting v26
for html_file in ['index.html', 'dashboard/templates/dashboard/index.html']:
    with open(html_file, 'r', encoding='utf-8') as f:
        html_content = f.read()
    html_content = html_content.replace('main.js?v=25', 'main.js?v=26')
    html_content = html_content.replace('style.css?v=25', 'style.css?v=26')
    with open(html_file, 'w', encoding='utf-8') as f:
        f.write(html_content)

print('Updated exportToPDF')
