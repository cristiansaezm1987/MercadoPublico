...
for filepath in files:
    ...
    # 1. Fix the renderQuoteResults UI bug (missing tabsHtml)
    ...
    if buggy_str in content:
        ...
        print(f"Fixed tabsHtml bug in {filepath}")

    # 2. Replace the exportToPDF function completely.
    # We find where it starts, and where the NEXT function starts.
    # The next function could be `window.saveBid` or `document.addEventListener`
    start_str = "window.exportToPDF = function() {"
    
    start_idx = content.find(start_str)
    if start_idx != -1:
        # Find the next top-level window. or document. call after start_idx
        next_func_idx = content.find("window.saveBid = function(", start_idx)
        if next_func_idx == -1:
            next_func_idx = content.find("document.addEventListener", start_idx)
            
        if next_func_idx != -1:
            # Reconstruct the new function
    ...
    """
            content = content[:start_idx] + new_pdf_func + content[next_func_idx:]
            print(f"Replaced exportToPDF in {filepath}")
...
for html_file in ['index.html', 'dashboard/templates/dashboard/index.html']:
    with open(html_file, 'r', encoding='utf-8') as f:
        html_content = f.read()
    html_content = html_content.replace('main.js?v=26', 'main.js?v=27')
    html_content = html_content.replace('style.css?v=26', 'style.css?v=27')
